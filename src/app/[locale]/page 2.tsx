                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-center">
                        ...
                        <div className="flex flex-col gap-6 text-center lg:text-left">
                            <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold leading-tight text-hypatia-charcoal">
                                {t("Hero.title1")}
                                <br />
                                <span className="text-[#661a91]">{t("Hero.title2")}</span>
                            </h1>
                            ...
                            <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
                                ...
                                <Dialog>
                                    ...
                                    <DialogContent>
                                        <DialogHeader>
                                            <DialogTitle className="sr-only">{t("JoinForm.title")}</DialogTitle>
                                        </DialogHeader>
                                        <JoinForm />
                                    </DialogContent>
                                </Dialog>
                ...
                <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="mt-16 md:mt-24 space-y-24">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-16 items-center">
                            <div className="relative order-last md:order-first">
                                <div className="relative z-10 md:transform md:-translate-x-8 p-8">
                                    <h3 className="text-2xl sm:text-3xl font-bold text-[hsl(var(--hypatia-soft-purple))]">{t("About.mission.title")}</h3>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-16 items-center">
                            ...
                            <div className="relative">
                                <div className="relative z-10 md:transform md:translate-x-8 p-8">
                                    <h3 className="text-2xl sm:text-3xl font-bold text-[hsl(var(--hypatia-soft-pink))]">{t("About.vision.title")}</h3>
                                    <p className="mt-4 text-lg text-muted-foreground">
                                        {t("About.vision.description1")}
                                        {" "}
                                        {t("About.vision.description2")}
                                        {" "}
                                        {t("About.vision.description3")}
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="mt-24 md:mt-32">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                            {coreValues.map(({ icon: Icon, titleKey, color, tint, border }) => (
                                <div
                                    key={titleKey}
                                ...
                                >
                                    ...
                                    <h4 className="text-xl font-bold mb-2" style={{ color }}>
                                        {t(`Values.${titleKey}.title`)}
                                    </h4>
                                    <p className="text-base text-muted-foreground">
                                        {t(`Values.${titleKey}.description`)}
                                    </p>
                                </div>
                            ))}
                        </div>
                            ...
                            >
                                {projects.map((p) => (
                                    <div key={p.slug} className="flex-shrink-0 w-[85vw] sm:w-96">
                                        <ProjectCard
                                            slug={p.slug}
                                            title={locale === 'en' ? p.en.title : p.tr.title}
                                            description={locale === 'en' ? (p.en.detail.heroSubtitle || p.en.description) : (p.tr.detail.heroSubtitle || p.tr.description)}
                                            imageUrl={p.imageUrl}
                                        />
                                    </div>
                                ))}
                                ...
                                <div className="transition-all duration-300 group-hover:opacity-0 group-hover:-translate-y-4">
                                    {member.imageUrl && (
                                        <div className="mx-auto mb-4 h-24 w-24 overflow-hidden rounded-full border-4 border-white shadow-lg">
                                            <Image
                                                src={member.imageUrl}
                                                alt={`${member.name} ${t("Team.imageAlt")}`}
                                                width={96}
                                                height={96}
