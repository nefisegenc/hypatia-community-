...
// 1. SERVER ACTION'I TANIMLIYORUZ
// Bu fonksiyon sadece sunucuda çalışacak.
async function submitApplication(formData: FormData) {
    "use server"; // Bu direktif, fonksiyonun bir Server Action olduğunu belirtir.

    // Formdan verileri `name` attribute'larına göre alıyoruz.
    const name = formData.get("name");
    const email = formData.get("email");
    const github = formData.get("github");
    const reason = formData.get("reason");

    // Şimdilik, verilerin sunucuya ulaştığını görmek için terminale yazdırıyoruz.
    ...
    // TODO: Gelecekte burada veritabanına kaydetme veya e-posta gönderme işlemleri yapılacak.
}
